...
> 💡Tailwind는 기본적으로 tailwindcss를 가져올 때 `--font-_`, `--color-_`, `--shadow-\*` 등과 같은 기본 테마 변수들을 포함하는 `theme.css` 파일을 불러온다. 이 덕분에 `bg-red-200`, `font-serif`, `shadow-sm`과 같은 유틸리티 클래스가 별도로 정의하지 않아도 사용 할 수 있다.
